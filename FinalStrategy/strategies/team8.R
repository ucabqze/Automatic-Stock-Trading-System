...
getOrders <- function(store, newRowList, currentPos, info, params){
  ...
  #######Calculate position#######################################
  if (store$iter > 50 && store$iter > params$cmoLookback && store$iter > params$bbandsLookback && store$iter > params$cciLookback) {
    for (i in 1:length(params$series)) {
      ...
      #######CCI#######
      cci = CCI(high_low_close, n = params$cciLookback)
      ...
      bbands = BBands(high_low_close,n=params$bbandsLookback,sd=params$bbandsSdParam)
      ...
      if(abs(cmo[store$iter])<params$cmoLowerBound){
        
        if ( cl < bbands[store$iter, "dn"]
             && cci[store$iter] > -100 && cci[store$iter] < 100
             && abs(store$bbandsDownCount[i]-store$bbandsUpCount[i])<params$bbandRiskCount ){ ###store$bbandsDownCount[i]<4
          
          pos[params$series[i]] <- store$posSizes[params$series[i]]
          
        }
        if (cl > bbands[store$iter, "up"] 
            &&  cci[store$iter] > -100 && cci[store$iter] < 100
            && abs(store$bbandsDownCount[i]-store$bbandsUpCount[i])<params$bbandRiskCount ) {###store$bbandsUpCount[i]<4
      ...
      #risk management
      if ( cl < bbands[store$iter, "dn"]) store$bbandsDownCount[i] = store$bbandsDownCount[i] + 1
      if (cl > bbands[store$iter, "up"]) store$bbandsUpCount[i] = store$bbandsUpCount[i] + 1
      
      
      if(cl > bbands[store$iter, "dn"]) store$bbandsDownCount[i] = 0
      if(cl < bbands[store$iter, "up"]) store$bbandsUpCount[i] = 0
