...
plotResults <- function(dataList, results, plotType='ggplot2', titleString=NULL) {
    ...
    # Aggregate fitness
    pfolioPdratio <- pdratio(pfolioPnL$CumPnL)
    ...
    pdRatioLists <- toplot
    pdRatioLists[,1] <- pdRatioLists[,1]-1000000
    ...
    p1 <- p1 + geom_hline(yintercept = 1000000, colour="red", linetype = "longdash") # start line
